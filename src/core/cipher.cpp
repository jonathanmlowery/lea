...
std::array<uint8_t, 32> cipher_block(const std::array<uint8_t, 32>& block,
    ...
    for (size_t i = 0; i < block.size(); i += 2) {
        uint16_t combined = (static_cast<uint16_t>(block [i + 1]) << 8)
                          | block [i];
    ...
    for (const operation& op : operations) {
        ...
        std::bitset<16> col;
        for (size_t r = 0; r < 16; ++r) { col [r] = grid [r][op.col]; }

        std::bitset<16> shifted_col = (col << op.colOffset)
                                    | (col >> (16 - op.colOffset));
        for (size_t r = 0; r < 16; ++r) { grid [r][op.col] = shifted_col [r]; }
    }
